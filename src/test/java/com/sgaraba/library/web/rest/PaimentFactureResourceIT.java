...
class PaimentFactureResourceIT {
    ...
    void createPaimentFacture() throws Exception {
        ...
        restPaimentFactureMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(paimentFacture))
            )
    ...
    void getAllPaimentFacturesWithEagerRelationshipsIsEnabled() throws Exception {
        when(paimentFactureRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPaimentFactureMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(paimentFactureRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void putNewPaimentFacture() throws Exception {
        ...
        restPaimentFactureMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedPaimentFacture.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(updatedPaimentFacture))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingPaimentFacture() throws Exception {
        ...
        restPaimentFactureMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paimentFacture.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(paimentFacture))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamPaimentFacture() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaimentFactureMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(paimentFacture)))
    ...
    void patchNonExistingPaimentFacture() throws Exception {
        ...
        restPaimentFactureMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paimentFacture.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(paimentFacture))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaimentFacture() throws Exception {
        ...
        restPaimentFactureMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(paimentFacture))
            )
            .andExpect(status().isMethodNotAllowed());
