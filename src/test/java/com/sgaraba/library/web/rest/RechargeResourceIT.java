...
class RechargeResourceIT {
    ...
    void createRecharge() throws Exception {
        ...
        // Create the Recharge
        restRechargeMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(recharge)))
    ...
    void getAllRechargesWithEagerRelationshipsIsEnabled() throws Exception {
        when(rechargeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRechargeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(rechargeRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void putNewRecharge() throws Exception {
        ...
        restRechargeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedRecharge.getId())
    ...
    void putNonExistingRecharge() throws Exception {
        ...
        restRechargeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, recharge.getId())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void putWithMissingIdPathParamRecharge() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restRechargeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(recharge)))
    ...
    void patchNonExistingRecharge() throws Exception {
        ...
        restRechargeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, recharge.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(recharge))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamRecharge() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restRechargeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(recharge)))
            .andExpect(status().isMethodNotAllowed());
