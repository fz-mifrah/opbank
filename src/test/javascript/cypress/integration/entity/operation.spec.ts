...
describe('Operation e2e test', () => {
  ...
  const operationSample = { numOperation: 'Ball Glens Granite', typeOperatin: 'intangible', montant: 76368 };
  ...
  describe('new Operation page', () => {
    ...
    it('should create an instance of Operation', () => {
      ...
      cy.get(`[data-cy="date"]`).type('2022-05-05T03:46').should('have.value', '2022-05-05T03:46');

      cy.get(`[data-cy="typeOperatin"]`).type('Brand IB').should('have.value', 'Brand IB');

      cy.get(`[data-cy="etatOperation"]`).type('Salad optical Cotton').should('have.value', 'Salad optical Cotton');

      cy.get(`[data-cy="montant"]`).type('88107').should('have.value', '88107');
