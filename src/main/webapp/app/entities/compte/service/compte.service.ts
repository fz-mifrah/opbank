...
export class CompteService {
  ...
  protected convertDateFromClient(compte: ICompte): ICompte {
    return Object.assign({}, compte, {
      dateOuvertur: compte.dateOuvertur?.isValid() ? compte.dateOuvertur.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateOuvertur = res.body.dateOuvertur ? dayjs(res.body.dateOuvertur) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((compte: ICompte) => {
        compte.dateOuvertur = compte.dateOuvertur ? dayjs(compte.dateOuvertur) : undefined;
      });
    }
    return res;
  }
