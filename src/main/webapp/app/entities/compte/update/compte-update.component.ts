...
export class CompteUpdateComponent implements OnInit {
  ...
  protected updateForm(compte: ICompte): void {
    ...
    this.ribsCollection = this.carteBancaireService.addCarteBancaireToCollectionIfMissing(this.ribsCollection, compte.rib);
    this.banquesSharedCollection = this.banqueService.addBanqueToCollectionIfMissing(this.banquesSharedCollection, compte.dateOuverture);
  }
    ...
    this.carteBancaireService
      ...
      .pipe(
        map((carteBancaires: ICarteBancaire[]) =>
          this.carteBancaireService.addCarteBancaireToCollectionIfMissing(carteBancaires, this.editForm.get('rib')!.value)
        )
      )
      .subscribe((carteBancaires: ICarteBancaire[]) => (this.ribsCollection = carteBancaires));

    this.banqueService
      .query()
      .pipe(map((res: HttpResponse<IBanque[]>) => res.body ?? []))
      .pipe(
        map((banques: IBanque[]) => this.banqueService.addBanqueToCollectionIfMissing(banques, this.editForm.get('dateOuverture')!.value))
      )
