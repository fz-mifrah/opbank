...
describe('Compte Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call rib query and add missing value', () => {
      const compte: ICompte = { id: 456 };
      const rib: ICarteBancaire = { id: 12922 };
      compte.rib = rib;

      const ribCollection: ICarteBancaire[] = [{ id: 97704 }];
      jest.spyOn(carteBancaireService, 'query').mockReturnValue(of(new HttpResponse({ body: ribCollection })));
      const expectedCollection: ICarteBancaire[] = [rib, ...ribCollection];
      ...
      expect(carteBancaireService.addCarteBancaireToCollectionIfMissing).toHaveBeenCalledWith(ribCollection, rib);
      expect(comp.ribsCollection).toEqual(expectedCollection);
    });

    it('Should call Banque query and add missing value', () => {
      const compte: ICompte = { id: 456 };
      const dateOuverture: IBanque = { id: 73188 };
      compte.dateOuverture = dateOuverture;
      ...
      const additionalBanques = [dateOuverture];
    ...
    it('Should update editForm', () => {
      const compte: ICompte = { id: 456 };
      const rib: ICarteBancaire = { id: 25178 };
      compte.rib = rib;
      const dateOuverture: IBanque = { id: 53696 };
      compte.dateOuverture = dateOuverture;
      ...
      expect(comp.ribsCollection).toContain(rib);
      expect(comp.banquesSharedCollection).toContain(dateOuverture);
    });
  });
