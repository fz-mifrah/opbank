...
export class OperationUpdateComponent implements OnInit {
  ...
  typeOperationsCollection: IRecharge[] = [];
  typeOperationsCollection: IPaimentFacture[] = [];
  ...
  protected updateForm(operation: IOperation): void {
    ...
    this.typeOperationsCollection = this.rechargeService.addRechargeToCollectionIfMissing(
      this.typeOperationsCollection,
      operation.typeOperation
    );
    this.typeOperationsCollection = this.paimentFactureService.addPaimentFactureToCollectionIfMissing(
      this.typeOperationsCollection,
      operation.typeOperation
    );
    ...
    this.rechargeService
      ...
      .pipe(
        map((recharges: IRecharge[]) =>
          this.rechargeService.addRechargeToCollectionIfMissing(recharges, this.editForm.get('typeOperation')!.value)
        )
      )
      .subscribe((recharges: IRecharge[]) => (this.typeOperationsCollection = recharges));
    ...
    this.paimentFactureService
      ...
      .pipe(
        map((paimentFactures: IPaimentFacture[]) =>
          this.paimentFactureService.addPaimentFactureToCollectionIfMissing(paimentFactures, this.editForm.get('typeOperation')!.value)
        )
      )
      .subscribe((paimentFactures: IPaimentFacture[]) => (this.typeOperationsCollection = paimentFactures));
