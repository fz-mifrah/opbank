...
describe('Client Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call nom query and add missing value', () => {
      ...
      const nom: ICompte = { id: 68593 };
      client.nom = nom;

      const nomCollection: ICompte[] = [{ id: 22544 }];
      jest.spyOn(compteService, 'query').mockReturnValue(of(new HttpResponse({ body: nomCollection })));
      const expectedCollection: ICompte[] = [nom, ...nomCollection];
      ...
      expect(compteService.addCompteToCollectionIfMissing).toHaveBeenCalledWith(nomCollection, nom);
      expect(comp.nomsCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const client: IClient = { id: 456 };
      const nom: ICompte = { id: 87770 };
      client.nom = nom;
